...
<cms:formatter var="content" val="value" rdfa="rdfa">
    ...
    <div class="panel panel-default ${cms.element.settings.boxschema}">

            <%-- Title of the text block --%>
        <c:if test="${not cms.element.settings.hidetitle}">
            <div class="panel-heading" ${rdfa.Title}>${value.Title}</div>
        </c:if>
            ...
            <c:forEach var="paragraph" items="${content.valueList.Paragraph}">
                <div class="list-group-item">
                    ...
                    <c:if test="${paragraph.value.Image.exists}">
                        ...
                        <c:set var="imgalign"><cms:elementsetting name="imgalign"
                                                                  default="${paragraph.value.Image.value.Align}"/></c:set>
                    ...
                    <c:if test="${showimg && (imgalign == 'lefthl' || imgalign == 'righthl' || imgalign == 'top')}">
                        ...
                        <cms:img src="${paragraph.value.Image.value.Image}" width="${imgwidth}" scaleColor="transparent"
                                 scaleType="0" cssclass="${imgclass}" alt="${paragraph.value.Image.value.Title}"
